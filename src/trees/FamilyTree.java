...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			throw new TreeException("Line is illegal: colon not found");
		...
		else
		{
			...
			if(parentNode==null){
				throw new TreeException("Line is illegal: error in data file");//If the parent node wasn't found, there must have been something wrong in the 
			}
				//data file. Throw an exception.
		}
		
		// Add child nodes to parentNode.
		for(String name:childrenArray) {//For each name in childrenArray, create a new node and add that node to parentNode.
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("Illegal argument: name 1 not found"); //Throw a TreeException with a useful message
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("Illegal argument: name 2 not found"); //Throw TreeException with a useful message
